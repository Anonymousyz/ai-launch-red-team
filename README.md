...
这是一个不依赖代码执行的 Agent 技能。把“我们的 AI 准备上线”一类的方案描述交给它，它会按 **8 条否决条件**和**七个维度**做结构化红队评审，生成一张可带进评审会的上线否决卡：哪些条件尚未满足，依据是方案中的哪句话，还要追问什么。
...
该示例触发两条否决：“自动执行退款”对应高风险决策无人工复核；“监控后面再补”说明没有差错处理或回滚负责人。完整输出见 [examples/01-refund-agent.md](examples/01-refund-agent.md)。
...
- **不打分，也不批准。**它列出结构性缺口和初步判断。需要可留档、可复核并生成报告的评估时，可使用完整工具链：
